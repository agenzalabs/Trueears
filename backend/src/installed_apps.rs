...
mod other_impl {
    ...
    /// Initialize cache - no-op on non-Windows platforms
    pub fn init_cache() {
        log::info!("Installed apps cache not implemented for this platform");
    }

    /// Search installed apps - returns empty on non-Windows platforms
    pub fn search_installed_apps(_query: &str) -> Vec<InstalledApp> {
        log::debug!("Installed apps search not implemented for this platform");
        Vec::new()
    }

    /// Get installed popular apps - returns empty on non-Windows platforms
    pub fn get_installed_popular_apps() -> Vec<InstalledApp> {
        log::debug!("Installed apps not implemented for this platform");
        Vec::new()
    }

    /// Force refresh - no-op on non-Windows platforms
    pub fn force_refresh_cache() {
        log::debug!("Cache refresh not implemented for this platform");
    }
}
